...
Description:
    ...
    This code produces Fig. 2b in the main text.
...
if __name__ == "__main__":
    ...
    qfi_list = np.zeros_like(tau_range)
    time_list = np.zeros_like(tau_range)
    ...
    for i, tau in enumerate(tau_range):
        ...
        qfi = 4 * all_pred.cpu().numpy()[:,3] / N ** 2
        max_qfi = np.max(qfi)

        time = np.argmax(qfi) * tau

        qfi_list[i] = max_qfi
        time_list[i] = time

    np.savetxt('./result/qfi_tau.csv', np.stack((tau_range, qfi_list), axis=1), delimiter=',')
    np.savetxt('./result/time_tau.csv', np.stack((tau_range, time_list), axis=1), delimiter=',')

    plt.plot(np.log10(tau_range), np.log10(qfi_list))
    plt.savefig('./result/qfi_tau.png')
    plt.close()
    plt.plot(np.log10(tau_range), np.log10(time_list))
    plt.savefig('./result/time_tau.png')
    plt.close()